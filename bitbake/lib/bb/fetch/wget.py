...
#               complete, nothing to see here..
            ...
            for (find, replace) in premirrors:
                ...
                if newuri != uri:
                    if not self.supports(newuri, d):
                        from bb.fetch import methods
                        from bb.fetch import NoMethodError

                        for m in methods:
                            if m.supports (newuri, d):
                                try: 
                                    m.go (d, [newuri])
                                    completed = 1
                                except:
                            ...
                            raise NoMethodError(uri)
                        if completed: break

                    elif fetch_uri(newuri, basename, dl, md5, parm, localdata):
                        completed = 1
                        break
