...
def exec_task(task, d):
    ...
    def execute(graph, item):
        if data.getVarFlag(item, 'task', d):
            ...
            try:
                ...
                data.delVarFlag (task, 'md5sum', d)
                md5sum = data.getVarFlag (task, 'md5sum', localdata)
                if md5sum:
                    data.setVarFlag (task, 'md5sum', md5sum, d)
    ...
    if not data.getVarFlag(task, 'nostamp', d):
        mkstamp(task, d)
    elif data.getVarFlag(task, 'md5sum', d):
        mkstamp(task, d)
        data.delVarFlag (task, 'md5sum', d)
...
def stamp_is_current(task, d, checkdeps = 1):
    ...
    if not stamp:
        return 0
    nostamp = data.getVarFlag(task, 'nostamp', d)
    if nostamp == "1":
        return 0
...
def mkstamp(task, d):
    ...
    if os.access(stamp, os.F_OK):
        os.remove(stamp)
    f = open(stamp, "w")
    md5sum = data.getVarFlag(task, 'md5sum', d)
    if md5sum:
        f.write (md5sum)
    f.close()
