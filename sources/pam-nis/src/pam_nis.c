...
#define HELPERTOOL SBINDIR"/pam-nis-helper"

#ifndef PAM_FAIL_DELAY
#define pam_fail_delay(x,y) 0
#endif 

#define PAM_NIS_DELAY 3000000
...
static int _parse_opt(context_t *c, int argc, const char **argv)
{
	for (; argc; argc--, argv++) {
		...
		if (!strcmp(*argv, "debug"))
			...
			c->opt_no_warn = 1;
		else if (!strncmp(*argv, "domain=", 7))
			strncpy(c->opt_domain, &(arg[7]), sizeof(c->opt_domain));
		else
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *ph, int flags, int argc, const char **argv)
{
	...
	static struct pam_message m[1] = { { msg_style: PAM_PROMPT_ECHO_OFF, msg : "Password: " } };
	...
	if (password) {
		...
		if (c.opt_use_first_pass) {
			pam_fail_delay(ph, PAM_NIS_DELAY);
			return PAM_AUTH_ERR;
		}
	}
