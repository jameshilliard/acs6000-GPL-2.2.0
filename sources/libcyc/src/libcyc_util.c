...
/* Is hostname/IP address configured locally?  Only works for IPv4
 * for now. */
int libcyc_iface_check_local_hn(const char *hostname)
{
	...
	int ret = FALSE;
        ...
        if ((error = getaddrinfo(hostname, NULL, &hints, &aitop)) != 0) {
                return(ERROR);
        }
		...
		if (libcyc_iface_check_local_ipv4(inet_ntoa(sa->sin_addr))) {
			ret = TRUE;
			break;
		}
        }
...
/* Return TRUE (1) if the IP address is configured locally in any
 * of the primary interfaces (e.g. eth0, eth1, bond0) and FALSE(0)
 * otherwise */
int libcyc_iface_check_local_ipv4(const char *ipaddr)
{
	static const char * const local_iface[] = {
		"eth0",
		"eth1",
		"bond0",
		NULL
	...
	for (i = 0; local_iface[i]; i++) {
		if (strncmp(ipaddr, libcyc_iface_get_ip_ipv4(local_iface[i]),
			    IP_ADDR_STR_LEN) == 0) return(TRUE);
	}
...
const char *libcyc_iface_get_ip_ipv4(const char *ifname)
{
	struct ifreq ifr;
...
double libcyc_get_uptime(void)
{
	...
	if ((fp=fopen(PROC_UPTIME_FILE, "r")) == NULL) return(ERROR);
	fscanf(fp,"%le %le\n", &uptime, &idletime);
	fclose(fp);
