...
static int create_named_socket(struct cyc_info *, const char *);

static int create_named_socket(struct cyc_info *cyc, const char *filepath)
{
	...
	if ((cyc == NULL) || (! (cyc->flags & CYC_INIT))) return(ERROR);
        ...
        strncpy(name.sun_path, filepath, sizeof(name.sun_path));
...
int libcyc_sock_init(struct cyc_info *cyc)
{
	...
	/* Remove old sock file first in case of previous crash;
	   it's ok if the call fails */
	unlink(cyc->sock_filepath);
	
	/* Make the socket */
	sock=create_named_socket(cyc, cyc->sock_filepath);
	chmod(cyc->sock_filepath, S_IRUSR|S_IWUSR);

	return(sock);
}

int libcyc_get_sock_from_sockfile(struct cyc_info *cyc, const char *filepath)
{
        struct sockaddr_un name;
        int sock=ERROR;
        size_t size;
