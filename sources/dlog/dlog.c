...
static rule_table* get_local_rule_table()
{
  if ( local_table == NULL )
  {
    pid_to_name( getpid(), proc_name, sizeof( proc_name ) / sizeof( char ) );
                      ...
                      const char* ctx, const char* format, va_list va )
{
  ...
  if ( outputFlag & DLOG_OUTPUT_SYSLOG )
  {
    ...
    // [RK]Nov/19/2007 - included these lines to fix problem when syslog-ng is restarted.
    // close socket with syslog-ng
    if (proc_syslog_fd != -1) {
	close(proc_syslog_fd); 
	proc_syslog_fd = -1;
    }
...
void __dlog_printf( const int level, const char* context, 
                    const char* format, ... )
{
  if ( __dlog_should_log( level, context ) )
  {
    va_list ap;
    va_start(ap, format);
    writelog( level, proc_name, getpid(), context, format, ap );
    va_end( ap );
  }
}
