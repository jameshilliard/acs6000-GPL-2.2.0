...
inline static int dlog_should_log(const int level, const char* context )
{
	const char *__tmp__ = context==NULL?__func__:context;

	return __dlog_should_log( level, __tmp__);
}
...
//
//-----------------------------------------------------------------------------
#define dlog_printf( level, context, format, arg... ) do { \
			const char *__tmp__ = context; \
        __dlog_printf( level, (__tmp__==NULL?__func__:context), format, ##arg);} while (0)
...
//-----------------------------------------------------------------------------
#define dlog_printbuf( level, context, label, buff, len ) do { \
		const char *__tmp__ = context; \
        __dlog_printbuf( level, (__tmp__==NULL?__func__:context), label, buff, len ); } while (0)
