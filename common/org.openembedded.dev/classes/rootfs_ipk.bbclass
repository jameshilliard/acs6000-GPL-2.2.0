...
real_do_rootfs () {
    ...
    if [ "X${EXTRA_IPK_DIR}" != "X" -a -d ${EXTRA_IPK_DIR} ]
    then
        if [ ! -e ${EXTRA_IPK_DIR}/Packages ]
        then
            if [ -w ${EXTRA_IPK_DIR} ]
            then
                ipkg-make-index -r ${EXTRA_IPK_DIR}/Packages -p ${EXTRA_IPK_DIR}/Packages -l ${EXTRA_IPK_DIR}/Packages.filelist -m ${EXTRA_IPK_DIR}
            else
                echo "Directory ${EXTRA_IPK_DIR} not indexed and not writable"
                false
            fi
        fi
        echo "src extra file:${EXTRA_IPK_DIR}" >> ${T}/ipkg.conf
    fi
