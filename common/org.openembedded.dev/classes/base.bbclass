...
python base_do_clean() {
    ...
    pkgs = bb.data.getVar('PACKAGES', d, 1)
    pkgs = pkgs.split()

    if bb.data.getVar('DEBIAN_NAMES', d, 1):
        ipk_dir = bb.data.getVar('DEPLOY_DIR_IPK', d, 1)
        debian_dir = bb.data.getVar('STAGING_PKGMAPS_DIR', d, 1)

        for pkg in pkgs:
            fn = "%s/%s_*.ipk" % (ipk_dir, pkg)
            os.system ("rm -f %s" % fn)
    ...
    for pkg in pkgs:
        fn = bb.data.expand('${STAGING_DIR}/pkgdata/%s' % pkg, d)
        os.system ("rm -f %s" % fn)
        fn = bb.data.expand('${STAGING_DIR}/pkgdata/runtime/%s' % pkg, d)
        os.system ("rm -f %s" % fn)
        fn = bb.data.expand('${STAGING_DIR}/pkgdata/runtime/%s.packaged' % pkg, d)
        os.system ("rm -f %s" % fn)
}
