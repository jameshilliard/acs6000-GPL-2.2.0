...
CYC_CVS ?= "williamc@cvs.cyclades.com/usr/cvsroot/"

SRC_URI = "ftp://ftp.kernel.org/pub/linux/libs/pam/pre/library/${p}.tar.gz \
 		cvs://${CYC_CVS};module=gsp/projects/sources/Linux-PAM-0.75 \
		file://install.patch;patch=1 \
		"
DEPENDS = "pwdb cracklib"
...
EXTRA_OECONF_alterpath += "--with-mailspool=no --enable-read-both-confs --enable-fakeroot=${D}  --enable-pamlocking --with-libcrack=no --disable-libcrack"
...
do_stage() {
    oe_runmake install
    cp -fpPR ${D}/${includedir}/security ${STAGING_INCDIR}
