...
GLIBC_ADDONS ?= "nptl"
GLIBC_EXTRA_OECONF ?= "--with-tls --with-__threads"
#GLIBC_EXTRA_OECONF ?= ""
...
SRC_URI = "ftp://ftp.gnu.org/gnu/glibc/glibc-${PV}.tar.gz \
	...
	file://etc/ld.so.conf \
	file://recursive.patch;patch=1 \
	file://generate-supported.mk"

# increase fd_set_size to 8192
SRC_URI_append_mergepoint = " file://select-limit.patch;patch=1 "

# Added additional Avocent buad rates to termios.h
...
glibc_do_unpack () {
	mv "${WORKDIR}/linuxthreads" "${WORKDIR}/linuxthreads_db" "${S}/" || true
	:
}
...
do_stage() {
	...
	if [ ${TARGET_SYS} = ${BUILD_SYS} ]; then
		ln -sf libc-${PV}.so ${CROSS_DIR}/${TARGET_SYS}/lib/libc.so.6
		ln -sf ld-${PV}.so ${CROSS_DIR}/${TARGET_SYS}/lib/ld.so
		ln -sf ld-${PV}.so ${CROSS_DIR}/${TARGET_SYS}/lib/ld-linux.so.2
	fi
}
