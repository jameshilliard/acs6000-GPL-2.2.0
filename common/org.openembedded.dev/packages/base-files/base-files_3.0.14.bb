...
SRC_URI_append_mergepoint = " file://setup.sh \
...
do_install_append_mergepoint () {
	install -m 0755 -d ${D}${base_bindir} ${D}${base_sbindir}
	
	install -m 0755 ${WORKDIR}/realroot.sh ${D}${base_bindir}
	install -m 0754 ${WORKDIR}/unconfigured.sh ${D}${base_sbindir}
	install -m 0754 ${WORKDIR}/setup.sh ${D}${base_sbindir}
}
...
FILES_${PN}_append_mergepoint = "${base_bindir}/realroot.sh ${base_sbindir}/*.sh"
...
CONFFILES_${PN}_GSP = "${sysconfdir}/fstab"
